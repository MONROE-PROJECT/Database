...
do
	...
	then
		...
		then
			...
			screen -dmS MonroeDB-${path} bash -c "python -u monroe_dbimporter.py --indir /experiments/monroe/${path} --failed /experiments/failed-retries/ --processed /experiments/processed/ --authenv  --host=127.0.0.1 --keyspace=monroe --interval=30  --verbosity=2 --concurrency=1 --recursive | tee importer_${path}.log"
		else
